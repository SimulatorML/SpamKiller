...
def recall_at_specificity(
    ...
    specificity = 1 - fpr
    index = binary_search_last_greater(specificity, min_specificity) #используем бинарный поиск для поиска последнего элемента
...
def bootstrap_metric(
    ...
    for index in tqdm(range(n_bootstraps), disable=not verbose):
        try:
            ...
            bootstraps_class_1 = np.random.choice(
                index_class_1, size=1)
            bootstrap = np.r_[bootstraps_class_1, bootstrap] #добавляем один индекс класса 1, из-за сильного дисбаланса 
...
def curves(true_labels: np.ndarray, pred_scores: np.ndarray) -> Tuple[np.ndarray]:
    ...
    pr_curve = PrecisionRecallDisplay.from_predictions(true_labels, pred_scores) #строим PrecisionRecall фигуру
    pr_curve = fig2numpy(pr_curve.figure_) #переводим PrecisionRecall фигуру в numpy.array

    roc_curve = RocCurveDisplay.from_predictions(true_labels, pred_scores) #строим RocCurve фигуру
