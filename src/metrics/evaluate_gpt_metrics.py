...
def save_predicted_data(
    data: pd.DataFrame, file_path: str, file_name: str, sep: str = ";"
):
...
def evaluate_metrics():
    ...
    batch = (
        2_000  # Leaving 500 tokens for computing 1 response (which is upper estimate)
    )
    response = []
    for i in range(
        0, len(X_test) + 1, batch
    ):  # Making predictions in parts to avoid TPM limit (40_000 TPM)
        start_time = time.time()
    ...
    all_pred_labels = [
        item["label"] for item in response
    ]  # Getting all the predicted and failed cases
    sucessfully_predicted = [
        True if item in [0, 1] else False for item in all_pred_labels
    ]  # Getting mask of those cases that were successfully predicted
    y_true = np.array(
        y_test[sucessfully_predicted], dtype="int64"
    )  # Making y_true for cases OpenAI managed to predict and not fail
    y_pred = np.array(
        [item["label"] for item in response if item["label"] is not None], dtype="int64"
    )  # Making y_pred for cases OpenAI managed to predict and not fail

    prompt_tokens = np.array(
        [item["prompt_tokens"] for item in response if item["label"] is not None]
    )  # If not error
    completion_tokens = np.array(
        [item["completion_tokens"] for item in response if item["label"] is not None]
    )
    time_spent = np.array(
        [item["time_spent"] for item in response if item["label"] is not None]
    )
    messages_quantity = len(X_test)

    logger.info(f"Messages quantity: {messages_quantity}")
    logger.info(
        f"Successfully managed to make a prediction for {len(y_pred)}/{messages_quantity} (all failed most likely reached TimeLimit)"
    )
    logger.info(
        f"Time spent on processing {messages_quantity} messages: {prediction_time_minutes} minutes"
    )

    # Save predictions to file
    data = pd.DataFrame(response)
    save_predicted_data(
        data=data, file_path="data/dataset", file_name="pred_test_gpt.csv"
    )
    ...
    logger.info(
        f"Mean money per output for spam-messages: {mean_worth_for_output_spam}"
    )
    logger.info(f"Mean worth per prediction: {mean_worth_per_prediction}")

    mean_latency = round(
        np.mean(time_spent), 2
    )  # Mean time spent on 1 prediction (seconds)
    ...
    logger.info(
        f"Mean latency for not-spam messages (time spent on 1 prediction): {mean_latency_not_spam} seconds"
    )
    logger.info(
        f"Mean latency for spam messages (time spent on 1 prediction): {mean_latency_spam} seconds"
    )
