...
async def handle_msg_with_args(
    ...
    # Проверка метки текущего сообщения
    if msg_features["label"] >= 1:  # Если сообщение определено как спам
        try:
            # Удаляем сообщение в любом случае
            await bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
            logger.info(f"Message {message.message_id} deleted successfully.")
            
            # Баним пользователя только если label == 2 (точно спам)
            if msg_features["label"] == 2:
                await bot.ban_chat_member(chat_id=message.chat.id, user_id=message.from_id)
                logger.info(f"User {message.from_id} was banned for spam.")
